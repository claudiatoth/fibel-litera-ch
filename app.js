/* ===========================================================
   ...
   17 secțiuni: Start · Litera ch · Suche ch · Cuvinte · Citește (cuvinte+propoziții) ·
   Legare silabe (unește) · Despărțire în silabe (clap) · Joc „are ch?" · Wo hörst du ch ·
...
const CH = {
  ...
  apfel: {de:'der Apfel', icon:'apfel', audio:'ch-apfel', gen:'m', art:'der', ro:'mărul',     m:false},
  ball:  {de:'der Ball',  icon:'ball',  audio:'ch-ball',  gen:'m', art:'der', ro:'mingea',    m:false},
  obst:  {de:'das Obst',  icon:'obst',  audio:'ch-obst',  gen:'n', art:'das', ro:'fructele',  m:false},
};
const WORDS_CH = ['buch','kuchen','dach','bach','koch','loch'];
const GAME_POOL= ['buch','maus','kuchen','katze','dach','ball'];
const ART_QUIZ = ['buch','kuchen','dach','bach'];
const GRID = [
  ['ch','m','a','ch','w'],['l','ch','M','o','n'],['ch','o','ch','b','ch'],['t','ch','a','u','F'],['d','ch','M','ch','u'],
];
/* despărțire în silabe (clap + audio pe silabă) — 1 → 2 → 3 silabe */
const SPLIT = [
...
const STORY = [
  ...
  {de:'<b class="m-bold">Herzchen</b> sieht ein <b class="m-bold">Buch</b>.',  ro:'Herzchen vede o carte.',         icon:'buch',     audio:'st-2'},
  {de:'<b class="m-bold">Herzchen</b> liest ein <b class="m-bold">Buch</b>.',  ro:'Herzchen citește o carte.',      icon:'buch',     audio:'st-3'},
  {de:'<b class="m-bold">Herzchen</b> ist glücklich.',                        ro:'Herzchen e fericit.',            icon:'herzchen', audio:'st-4'},
];
const QUIZ = [
  {type:'audio',   ans:'buch',   opts:['buch','maus','katze']},
  {type:'findm',   ans:'kuchen', opts:['kuchen','katze','ball']},
  {type:'article', word:'dach'},
  {type:'audio',   ans:'koch',   opts:['koch','apfel','maus']},
  {type:'findm',   ans:'bach',   opts:['bach','obst','ball']},
];
const LETTER = 'ch';
/* ch se aude la SFÂRȘIT (Buch, Dach, Bach) sau MIJLOC (Kuchen) — niciodată la început */
const POS = [
  {icon:'buch',   audio:'ch-buch',   pos:'e'},
  {icon:'dach',   audio:'ch-dach',   pos:'e'},
  {icon:'bach',   audio:'ch-bach',   pos:'e'},
  {icon:'kuchen', audio:'ch-kuchen', pos:'m'},
];
...
SCREENS[0] = function(){
  ...
  document.getElementById('startBtn').onclick = ()=>{ play('ch-wie-buch'); go(1); };
...
SCREENS[1] = function(){
  ...
  document.getElementById('bMaus').onclick = ()=>play('ch-wie-buch');
  setTimeout(()=>play('ch-wie-buch'), 350);
  wireNext(2);
};
...
SCREENS[2] = function(){
  ...
  GRID.forEach(row=> row.forEach(ch=>{
    const isM = (ch==='ch'); if(isM) total++;
    cells += '<div class="lcell" data-m="'+(isM?1:0)+'">'+ch+'</div>';
  }));
  show(
    '<p class="kicker">Suche ch 🔍</p>' +
    '<h1 class="title" style="font-size:1.4rem">Atinge toți „ch"</h1>' +
...
SCREENS[4] = function(){
  ...
  show(
    ...
    '<h1 class="title" style="font-size:1.1rem">Cu literele învățate + ch</h1>' +
...
/* 7 — JOC: Wo hat „ch"? */
SCREENS[7] = function(){
  let tiles = spreadShuffle(GAME_POOL, k=>CH[k].m, 2).map(k=>
    '<div class="tile" data-k="'+k+'" style="position:relative"><img src="'+ICON+CH[k].icon+'.svg" alt="'+CH[k].de+'"></div>'
  ).join('');
  show(
    hero('small') +
    '<p class="game-q">Atinge ce ARE sunetul <span class="m-hi">ch</span>!</p>' +
    '<div class="game-grid">'+tiles+'</div>' +
    '<p class="parent-note">Welches Wort hat „ch"? (Buch, Kuchen, Dach …)</p>'
  );
  let found = 0; const need = GAME_POOL.filter(k=>CH[k].m).length;
  stage.querySelectorAll('.tile').forEach(el=>{
    el.onclick = function(){
      const w = CH[el.dataset.k];
...
/* 8 — WO HÖRST DU das ch? (la sfârșit/mijloc) */
SCREENS[8] = function(){
  let qi = 0;
  function round(){
    const it = POS[qi];
    show(
      hero('small') +
      '<p class="kicker">Wo hörst du das <span class="m-hi">ch</span>? 👂</p>' +
      '<div class="circle" style="margin:0 auto;border-color:var(--auriu)"><img src="'+ICON+it.icon+'.svg" alt=""></div>' +
  ...
  function card(){
    ...
    show(
      ...
      '<div class="flashnav">' +
        ...
        '<span class="counter">'+(i+1)+' / '+WORDS_CH.length+'</span>' +
    ...
    nx.onclick = ()=>{ if(i<WORDS_CH.length-1){ i++; card(); } };
    wireNext(12);
  }
  card();
};

/* 12 — TRASEAZA „ch" — cu SĂGEȚI */
SCREENS[12] = function(){
  show(
    '<p class="kicker">Schreiben · ① urmează săgețile verzi ✏️</p>' +
    '<h1 class="title" style="font-size:1.2rem">Trasează „ch"</h1>' +
    '<div class="trace-wrap"><img class="trace-stroke" src="'+ICON+'stroke-ch.svg" alt="">' +
...
/* 14 — SCRIE PROPOZIȚIA */
SCREENS[14] = function(){
  const GLYPH = {'N':'s-cap-n','S':'s-cap-s','R':'s-cap-r','L':'s-cap-l','M':'s-cap-m','T':'s-cap-t','O':'s-cap-o','E':'s-cap-e','F':'s-cap-f','U':'s-cap-u','B':'s-cap-b','D':'s-cap-d','H':'s-cap-h','G':'s-cap-g',
                 'n':'s-n','s':'s-s','r':'s-r','t':'s-t','m':'s-m','a':'s-a','i':'s-i','l':'s-l','o':'s-o','e':'s-e','u':'s-u','b':'s-b','d':'s-d','h':'s-h','g':'s-g','c':'s-c'};
  const SENT = [
    {t:'Tom hat ein Buch.', a:'st-tomhatbuch'},
    {t:'Udo hat ein Dach.', a:'st-udohatdach'}
  ];
  ...
  show(
    ...
    '<p class="parent-note" style="margin-top:0">Fiecare literă cu săgețuța ei. „ch" = c + h. Trasează cu degetul peste literele cenușii.</p>' +
    rows +
  ...
  function q(){
    ...
    if (it.type==='audio'){
      body = '<p class="game-q">Ascultă 🔊 și atinge imaginea</p>' +
        ...
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+CH[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else if (it.type==='findm'){
      body = '<p class="game-q">Care ARE sunetul <span class="m-hi">ch</span>?</p>' +
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+CH[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else {
      const w = CH[it.word];
    ...
    if (it.type==='audio'){
      ...
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (el.dataset.k === it.ans){ el.classList.add('good'); play(CH[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else if (it.type==='findm'){
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (CH[el.dataset.k].m){ el.classList.add('good'); play(CH[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else {
      const w = CH[it.word]; play(w.audio);
...
SCREENS[16] = function(){
  show(
    ...
    '<div class="story" style="max-width:380px"><div class="story-row read" id="rima">' +
      '<img src="'+ICON+'buch.svg" alt="">' +
      '<div><div class="s-de"><b class="m-bold">ch</b> wie Bu<b class="m-bold">ch</b>.<br><b class="m-bold">Herzchen</b> liest ein <b class="m-bold">Buch</b>.</div>' +
      '<div class="s-ro">ch ca în Buch. Herzchen citește o carte.</div></div>' +
...
function printDiploma(){
  ...
  document.getElementById('diploma-print').innerHTML =
    '<div class="dip">' +
      ...
      '<div class="dip-text">Du hast gelernt:</div>' +
      '<div class="dip-letter">ch</div>' +
      '<div class="dip-text">ch wie Buch — mit Herzchen! 🧡</div>' +
...
function printColoring(){
  ...
  document.getElementById('coloring-print').innerHTML =
    '<div class="color-page">' +
      ...
      '<h2 class="cp-title">ch — Herzchen liest ein Buch</h2>' +
      '<img src="'+ICON+'herzchen-color.svg" alt="Herzchen liest ein Buch">' +
      '<div class="cp-sentence">Herzchen liest ein Buch.</div>' +
...
(function initParent(){
  ...
  if (wl) wl.innerHTML = WORDS_CH.map(k=>'<span>'+CH[k].de+'</span>').join('');
